...
import { systemQueue, dbQueue, deliverQueue, inboxQueue, objectStorageQueue, endedPollNotificationQueue, webhookDeliverQueue, subscriptionQueue } from './queues.js';
...
export default function() {
	...
	systemQueue.add('approvePendingContent', {
	}, {
	...
	// Check for LMS auto-sync every hour (respects individual school sync frequency)
	systemQueue.add('lmsAutoSync', {
	}, {
		repeat: { cron: '0 * * * *' },
		removeOnComplete: true,
	});

	// Process graduations daily at midnight
	systemQueue.add('processGraduations', {
	}, {
		repeat: { cron: '0 0 * * *' },
		removeOnComplete: true,
	});

	// Process graduated student deletions daily at 1 AM
	systemQueue.add('processGraduatedDeletions', {
	}, {
		repeat: { cron: '0 1 * * *' },
		removeOnComplete: true,
	});
