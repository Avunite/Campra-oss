...
async function save(file: DriveFile, path: string, name: string, type: string, hash: string, size: number): Promise<DriveFile> {
	...
	if (meta.useObjectStorage) {
		...
		// for original
		const fileUuid = uuid();
		const key = `${meta.objectStoragePrefix}/${fileUuid}${ext}`;
		...
		if (alts.webpublic) {
			webpublicKey = `${meta.objectStoragePrefix}/webpublic-${fileUuid}.${alts.webpublic.ext}`;
		...
		if (alts.thumbnail) {
			thumbnailKey = `${meta.objectStoragePrefix}/thumbnail-${fileUuid}.${alts.thumbnail.ext}`;
		...
		const internalFileUuid = uuid();
		const accessKey = internalFileUuid;
		const thumbnailAccessKey = 'thumbnail-' + internalFileUuid;
		const webpublicAccessKey = 'webpublic-' + internalFileUuid;
...
export async function addFile({
	...
	if (user && !isLink) {
		...
		if (info.size > maxFileSize) {
			throw new IdentifiableError('f5f2b2d8-35c9-4ac9-8375-f8e01d62e706',
				`File size exceeds the limit. ${isPlus ? 'Plus users' : (isMPlus ? 'Mini Campra+ Users' : 'Users')} can upload files up to ${isPlus ? '5GB' : (isMPlus ? '1Gb' : '500MB')}.`);
		}
	}
	//#endregion
	...
	// Submit image to Iffy for moderation (async - results come via webhook)
	if (user && file.type.startsWith('image/')) {
		// Don't await this - let it happen in background
		contentAutoModerator.moderateImage(file.id, file.url, user.id).catch(error => {
			logger.warn('Image content moderation submission failed:', error instanceof Error ? error.message : String(error));
		});
	}
