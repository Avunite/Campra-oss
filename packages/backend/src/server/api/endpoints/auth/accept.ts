...
export default define(meta, paramDef, async (ps, user) => {
	...
	if (app.oauth2 && session.redirectUri) {
		...
		const authorizationCode = secureRndstr(32);

		// Set expiration time for the authorization code (10 minutes)
		const now = new Date();
		...
		// Return authorization code to be used by the client
		return {
			authorizationCode,
			redirectUri: `${session.redirectUri}${session.redirectUri.includes('?') ? '&' : '?'}code=${authorizationCode}`,
		};
	}
