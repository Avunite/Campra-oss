...
import { AbuseUserReports, Users, Schools } from '@/models/index.js';
	...
	setImmediate(async () => {
		...
		if (user.schoolId) {
			const schoolAdmins = await Users.find({
				where: {
					adminForSchoolId: user.schoolId,
					isSchoolAdmin: true,
				},
			});
		...
		if (user.schoolId) {
			...
			if (school && school.domain) {
				const schoolAdminEmail = `admin@${school.domain}`;
				sendEmail(schoolAdminEmail, `New abuse report for student: ${user.username}`,
					`A new abuse report has been submitted for ${user.username} (${user.id}).\n\nReport details:\n${ps.comment}`,
