...
export const paramDef = {
	...
	properties: {
		...
		preReleaseAllowedRoles: { type: 'array', nullable: true, items: {
			type: 'string',
		} },
		preReleaseAllowedUserIds: { type: 'array', nullable: true, items: {
			type: 'string',
		} },
		...
		proxyAccountId: { type: 'string', format: 'campra:id', nullable: true },
		automodAccountId: { type: 'string', format: 'campra:id', nullable: true },
		maintainerName: { type: 'string', nullable: true },
		maintainerEmail: { type: 'string', nullable: true },
		pinnedPages: { type: 'array', items: {
			type: 'string',
		} },
		pinnedClipId: { type: 'string', format: 'campra:id', nullable: true },
		...
		// Content moderation (iffy) settings
		enableContentModeration: { type: 'boolean' },
		iffyApiKey: { type: 'string', nullable: true },
		iffyApiUrl: { type: 'string', nullable: true },
		iffyWebhookSecret: { type: 'string', nullable: true },
		iffyConfidenceThreshold: { type: 'string', enum: ['low', 'medium', 'high'] },
		autoHideInappropriateContent: { type: 'boolean' },
		enableSchoolContentModeration: { type: 'boolean' },
		// Stripe settings
		stripe_key: { type: 'string', nullable: true },
		stripe_webhook_secret: { type: 'string', nullable: true },
		stripeSchoolPriceId: { type: 'string', nullable: true },
	},
	required: [],
} as const;
...
export default define(meta, paramDef, async (ps, me) => {
	...
	if (ps.automodAccountId !== undefined) {
		set.automodAccountId = ps.automodAccountId;
	}
	...
	// Content moderation (iffy) settings
	if (ps.enableContentModeration !== undefined) {
		set.enableContentModeration = ps.enableContentModeration;
	}

	if (ps.iffyApiKey !== undefined) {
		if (ps.iffyApiKey === '') {
			set.iffyApiKey = null;
		} else {
			set.iffyApiKey = ps.iffyApiKey;
		}
	}

	if (ps.iffyApiUrl !== undefined) {
		if (ps.iffyApiUrl === '') {
			set.iffyApiUrl = null;
		} else {
			set.iffyApiUrl = ps.iffyApiUrl;
		}
	}

	if (ps.iffyWebhookSecret !== undefined) {
		if (ps.iffyWebhookSecret === '') {
			set.iffyWebhookSecret = null;
		} else {
			set.iffyWebhookSecret = ps.iffyWebhookSecret;
		}
	}

	if (ps.iffyConfidenceThreshold !== undefined) {
		set.iffyConfidenceThreshold = ps.iffyConfidenceThreshold;
	}

	if (ps.autoHideInappropriateContent !== undefined) {
		set.autoHideInappropriateContent = ps.autoHideInappropriateContent;
	}

	if (ps.enableSchoolContentModeration !== undefined) {
		set.enableSchoolContentModeration = ps.enableSchoolContentModeration;
	}

	if (ps.stripe_key !== undefined) {
		set.stripeKey = ps.stripe_key;
	}

	if (ps.stripe_webhook_secret !== undefined) {
		set.stripeWebhookSecret = ps.stripe_webhook_secret;
	}

	if (ps.stripeSchoolPriceId !== undefined) {
		set.stripeSchoolPriceId = ps.stripeSchoolPriceId;
	}
