...
const graduationDateSchema = { type: 'string', pattern: /^([0-9]{4})-([0-9]{2})-([0-9]{2})$/.toString().slice(1, -1) } as const;
...
export const UserRepository = db.getRepository(User).extend({
	...
	validateGraduationDate: ajv.compile(graduationDateSchema),
	//#endregion
	...
	async getAvatarUrl(user: User): Promise<string> {
		if (user.avatar) {
			return await DriveFiles.getSecureUrl(user.avatar, true) || this.getIdenticonUrl(user.id);
		} else if (user.avatarId) {
			const avatar = await DriveFiles.findOneByOrFail({ id: user.avatarId });
			return await DriveFiles.getSecureUrl(avatar, true) || this.getIdenticonUrl(user.id);
		} else {
			return this.getIdenticonUrl(user.id);
		}
	},

	async getAvatarUrlAsync(user: User): Promise<string> {
		if (user.avatar) {
			return await DriveFiles.getSecureUrl(user.avatar, true) || this.getIdenticonUrl(user.id);
		} else {
			return this.getIdenticonUrl(user.id);
	...
	async pack<ExpectsMe extends boolean | null = null, D extends boolean = false>(
		...
		// Load school information if the user has a school - FERPA compliance check
		const school = user.schoolId && (
			// Only show school info if:
			// 1. It's the user themselves
			// 2. Viewer is admin/moderator/school admin
			// 3. Viewer is from the same school
			isMe || 
			(me && (me.isAdmin || me.isModerator || me.isSchoolAdmin)) ||
			(me && me.schoolId === user.schoolId)
		) ? await import('@/models/index.js').then(async ({ Schools, DriveFiles }) => {
			const schoolData = await Schools.findOneBy({ id: user.schoolId });
			if (!schoolData) return null;

			// Resolve logo URL from logoId if present
			let logoUrl = schoolData.logoUrl;
			if (schoolData.logoId && !logoUrl) {
				try {
					const logoFile = await DriveFiles.findOneBy({ id: schoolData.logoId });
					if (logoFile) {
		...
		const packed = {
			...
			host: user.host,
			avatarUrl: await this.getAvatarUrlAsync(user),
			avatarBlurhash: user.avatar?.blurhash || null,
			avatarColor: null, // 後方互換性のため
			isAdmin: user.isAdmin || falsy,
			isModerator: user.isModerator || falsy,
			isSchoolAdmin: user.isSchoolAdmin || falsy,
			adminForSchoolId: user.adminForSchoolId || null,
			...
			// CAMPRA PHASE 2: Include school information for @name@school.edu format
			school: school ? {
				id: school.id,
				name: school.name,
			...
			avatarDecorations: await Promise.all(user.avatarDecorations.map(async decoration => {
				...
				const packed = await Decorations.pack(dec);
				return {
					...decoration,
					url: packed.url,
				};
			...
			...(opts.detail ? {
				...
				bannerUrl: user.banner ? await DriveFiles.getSecureUrl(user.banner, false) : null,
			...
			...(opts.detail && isMe ? {
				...
				mustChangePassword: user.mustChangePassword || falsy,
