...
<MkContainer :show-header="widgetProps.showHeader" :style="`height: ${widgetProps.height}px;`" :scrollable="true" class="mkw-timeline">
	<template #header>
		<button class="_button" @click="choose">
			...
			<i v-else-if="widgetProps.src === 'school'" class="ph-graduation-cap-bold ph-lg"></i>
...
const choose = async (ev) => {
	...
	}, ...($i && ($i.schoolId || $i.school?.id) ? [{
		text: i18n.ts._timelines.school || 'School',
		icon: 'ph-graduation-cap-bold ph-lg',
		action: () => { setSrc('school'); }
	}] : []), {
