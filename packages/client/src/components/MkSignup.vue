...
<form class="qlvuhzng _formRoot" autocomplete="new-password" @submit.prevent="onSubmit">
	...
	<div v-if="!instance.disableRegistration || (instance.disableRegistration && invitationState === 'entered')">
		<MkInput v-model="username" class="_formBlock" type="text" pattern="^[a-zA-Z0-9_]{1,20}$" :spellcheck="false" :readonly="usernameReadonly" required data-cy-signup-username @update:modelValue="onChangeUsername">
			...
			<template #suffix v-if="emailDomain">@{{ emailDomain }}</template>
			<template #caption>
				<span v-if="usernameReadonly" style="color: var(--accent)"><i class="ph-info-bold ph-lg ph-fw ph-lg"></i> {{ i18n.ts.usernameAutoGenerated || 'Username will be automatically generated from your email' }}</span>
				<span v-else-if="usernameState === 'wait'" style="color:#6e6a86"><i class="ph-circle-notch-bold ph-lg fa-pulse ph-fw ph-lg"></i> {{ i18n.ts.checking }}</span>
		...
		<SchoolCheck v-if="instance.emailRequiredForSignup && email" :email="email" @school-info="onSchoolInfo" />
...
import SchoolCheck from './auth/school-check.vue';
...
let usernameReadonly = $ref(false);
...
const emailDomain = $computed((): string | null => {
...
function onChangeUsername(): void {
	...
	os.api('username/available', {
		...
		if (!result.available) {
			...
			// If available, we'll show it as OK
			// In a real implementation, you might want to validate against name patterns here too
			// when you have access to the user's real name from school registration
			usernameState = 'ok';
		}
	}).catch(() => {
		usernameState = 'error';
...
function onSchoolInfo(info: any): void {
	schoolInfo = info;
	
	// If school doesn't allow custom usernames, auto-populate from email
	if (schoolInfo?.school && schoolInfo.school.allowStudentsChooseUsername === false) {
		usernameReadonly = true;
		const emailPrefix = email.split('@')[0];
		// Sanitize username to only include valid characters
