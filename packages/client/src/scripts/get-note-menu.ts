...
export function getNoteMenu(props: {
	...
	if ($i) {
		...
		...($i.isModerator || $i.isAdmin ? [
			...
			...(appearNote.userId === $i.id || $i.isModerator || $i.isAdmin || ($i.isSchoolAdmin && $i.adminForSchoolId === appearNote.user.schoolId) ? [
				null,
				appearNote.userId === $i.id ? {
				...
				} : undefined,
				{
					...
					text: i18n.ts.delete,
					danger: true,
					action: appearNote.userId === $i.id || $i.isModerator || $i.isAdmin ? del : () => {
						os.confirm({
							type: 'warning',
							text: `${i18n.ts.noteDeleteConfirm}\n\nAs a school administrator, you are deleting a post from a student in your school.`,
						}).then(({ canceled }) => {
							if (canceled) return;
							
							os.apiWithDialog('school/delete-post', {
								postId: appearNote.id,
							}).then(() => {
								props.isDeleted.value = true;
							});
						});
