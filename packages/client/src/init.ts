...
(async () => {
	...
	if (loginId) {
		...
		history.replaceState({ campra: 'loginId' }, '', target);
	}
	...
	const rootEl = (() => {
		const MISSKEY_MOUNT_DIV_ID = 'campra_app';
	...
	// Check if user must change password (for school admins with temporary passwords)
	if ($i && $i.mustChangePassword) {
		import('@/os').then(({ popup }) => {
			// Show forced password change modal
			popup(defineAsyncComponent(() => import('@/components/MkForcedPasswordChangeModal.vue')), {}, {
				success: () => {
					// Refresh account after successful password change
					refreshAccount();
				},
			}, 'closed');
		});
