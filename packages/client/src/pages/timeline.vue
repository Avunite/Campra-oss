<template>
	...
	const isGlobalTimelineAvailable =
		...
		($i != null && ($i.isModerator || $i.isAdmin));
	const isSchoolTimelineAvailable = $i != null && ($i.schoolId || $i.school?.id);
	const keymap = {
		t: focus,
	};
	...
	if (isSchoolTimelineAvailable) {
		timelines.push('school');
	}
	...
	let queue = $ref(0);
	const src = $computed({
		get: () => defaultStore.reactiveState.tl.value.src || 'social',
		set: (x) => {
			saveSrc(x);
	...
	function saveSrc(
		newSrc: 'social' | 'home' | 'school' | 'local' | 'global' | 'recommended',
	): void {
		defaultStore.set('tl', {
...
const headerTabs = $computed(() => [
    ...
    ...(isSchoolTimelineAvailable
        ? [
            {
                key: 'school',
                title: i18n.ts._timelines.school || 'School',
...
definePageMetadata(
	...
	function onSlideChange() {
		saveSrc(timelines[swiperRef.activeIndex] as any);
	}
